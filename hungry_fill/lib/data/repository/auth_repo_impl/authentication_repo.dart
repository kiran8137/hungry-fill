...
class AuthenticationRepoImplement extends AuthRepository {
  ...
  AuthenticationRepoImplement({required this.firestore , required this.firebaseauth});

  @override
  Future<void> sendOtp(
      {required String phoneNumber, required BuildContext context , required String? username , required String? useremail}) async {
    try {
  ...
  @override
  Future<UserCredential> verifyOtp({required String verificationId, required String otpCode }) async {
     
     
     try{
      final credentials = PhoneAuthProvider.credential(verificationId: verificationId, smsCode: otpCode);
      
      Future<UserCredential> userdetail =  firebaseauth.signInWithCredential(credentials);
...
@override
  Future<bool> checkUserAlreadyRegistered({required String? phonenumber}) async{

    try{
      
    final result = await firestore.collection("Users").where("userMobileNumber",isEqualTo: phonenumber).get();
...
@override
  Future<void> sendOtpLogInPage({required String? phonenumber}) async{
