...
class DishRepoImpl extends DishRepository {
  ...
  Future<List<DishModel>> getDish({required String? resuerid}) async {
    try {
      final snapshot = await FirebaseFirestore.instance
          ...
          .collection("Dishes")
          .get();
      List<DishModel> dish = snapshot.docs
  ...
  Future<List<DishModel>> searchDishes(
    ...
    try {
      ...
      List<DishModel> dishes = snapshot.docs
          .map((dish) => DishModel.fromJson(json: dish.data()))
          .toList();
  ...
  @override
  Future<List<DishModel>> getCategoryDishes(
      {required String categoryid, required String resuerid}) async {
    try {
      final snapshot = await FirebaseFirestore.instance
          .collection("Restaurants")
          .doc(resuerid)
  ...
  @override
  Future<List<RestaurantAndDish>> getFilterDish(
      {required String dishcategoryid}) async {
  ...
  Future<List<DishCategoryModel>> getDishesCategoryFilterOptionFromDb() async {
    try {
      final snapshot =
          await FirebaseFirestore.instance.collection('DishCategories').get();
      final result = snapshot.docs
          .map((doc) => DishCategoryModel.fromJson(json: doc.data()))
          .toList();
